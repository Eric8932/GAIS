...
def logprobs_from_logits(logits, labels, gather=True):
    """
    See: https://github.com/huggingface/trl/blob/v0.8.6/trl/core.py
    """
    ...
    # Check if labels are the same shape as logits (shifted) or just indices
    if labels.dim() == logits.dim() - 1:
        labels = labels.unsqueeze(-1)
        
    logpy = logp.gather(-1, labels).squeeze(-1)
    return logpy
